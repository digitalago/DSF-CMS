  ...
  foreach (array_keys($section_storage_manager->getDefinitions()) as $section_storage_type) {
    ...
    foreach ($key_value->getAll() as $key => $value) {
      ...
      if ($section_storage = $section_storage_manager->load($section_storage_type, $contexts)) {
        ...
        // Tempstored sections created before contexts were required will
        // be malformed when unserialized and will need to be removed to avoid
        // errors when editing any layouts that had a tempstore entry.
        if (!$value->data['section_storage']->getContextValues()) {
